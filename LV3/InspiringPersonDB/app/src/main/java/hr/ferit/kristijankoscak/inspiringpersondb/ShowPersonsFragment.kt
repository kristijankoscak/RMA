    ...
    private fun displayData() {
        val personListener = object: PersonInteractionListener{
            override fun onShowDetails(id: Int) {
                val person = inspiringPersonDao.getByID(id)
                val quotes: List<String> = person!!.quote.split(";")
                val randomNumber : Int = Random().nextInt(quotes.size)
                Toast.makeText(activity, quotes[randomNumber], Toast.LENGTH_SHORT).show()
            }
            override fun removePerson(id: Int) {
                inspiringPersonDao.delete(inspiringPersonDao.getByID(id))
                (personDisplay.adapter as PersonAdapter).refreshData(inspiringPersonDao.getAll())
            }
        ...
        personDisplay.adapter = PersonAdapter(inspiringPersonDao.getAll(),personListener)
    }
}